(function (exports) {
  ...
  Player.prototype._makef = function (delta, v, a) {
    ...
    return function (x) {
      return (
        (Math.exp(x) * (Math.pow(delta, 2) - Math.pow(pl.__rd, 2) - v - Math.exp(x))) /
          (2 * Math.pow(Math.pow(pl.__rd, 2) + v + Math.exp(x), 2)) -
        (x - a) / Math.pow(pl._tau, 2)
      );
    };
  };
  ...
  var volatility_algorithms = {
    oldprocedure: function (v, delta) {
      ...
      function new_sigma(sigma, phi, v, delta, tau) {
        ...
        while (x != old_x) {
          ...
          var h2 =
            -1 / Math.pow(tau, 2) -
            (0.5 * Math.exp(old_x) * (Math.pow(phi, 2) + v)) / Math.pow(d, 2) +
            (0.5 * Math.pow(delta, 2) * Math.exp(old_x) * (Math.pow(phi, 2) + v - Math.exp(old_x))) / Math.pow(d, 3);
          x = old_x - h1 / h2;
        }
        return Math.exp(x / 2);
      }
      ...
      function Dequation(phi, v, x, tau, delta) {
        ...
        return (
          -1 / Math.pow(tau, 2) -
          (0.5 * Math.exp(x)) / d +
          (0.5 * Math.exp(x) * (Math.exp(x) + Math.pow(delta, 2))) / Math.pow(d, 2) -
    ...
    newprocedure: function (v, delta) {
      ...
      while (Math.abs(B - A) > epsilon) {
        ...
        fC = f(C);
        if (fC * fB <= 0) {
          // March 22, 2022 algorithm update : `<` replaced by `<=`
          A = B;
          fA = fB;
        } else {
          fA = fA / 2;
    ...
    oldprocedure_simple: function (v, delta) {
      ...
      while (Math.abs(x0 - x1) > 0.00000001) {
        ...
        h2 =
          -1 / Math.pow(tau, 2) -
          (0.5 * Math.exp(x0) * (Math.pow(this.__rating, 2) + v)) / Math.pow(d, 2) +
          (0.5 * Math.pow(delta, 2) * Math.exp(x0) * (Math.pow(this.__rating, 2) + v - Math.exp(x0))) / Math.pow(d, 3);
        x1 = x0 - h1 / h2;
      }
